...
export default defineConfig(({ mode }) => {
  ...
  const emitSourcemaps = mode === 'development' || process.env.FIGMA_PUBLIC_URL === 'https://verificatorapi-ancahaepeadghgex.southeastasia-01.azurewebsites.net'
  ...
  return {
    ...
    server: {
      ...
      watch: { ignored: ['**/.figma/**'] },
      proxy: {
        // Local: browser → Vite → verificatorapi (same-origin, no CORS).
        // Production: server.js proxies /api the same way.
        '/api': {
          target:
            process.env.API_PROXY_TARGET ||
            'https://verificatorapi-ancahaepeadghgex.southeastasia-01.azurewebsites.net',
          changeOrigin: true,
          secure: false,
        },
      },
